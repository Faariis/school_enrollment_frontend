...
const PDFDocument = ({ students, specialScoreNames, courseId, fullCourseName }) => {
  ...
  const firstPageStudents = students.slice(0, 22);
  const secondPageStudents = students.slice(22);
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page} orientation="landscape">
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            ...
            {firstPageStudents.map((student, studentIndex) => (
      ...
      {/* Second Page */}
      {secondPageStudents.length > 0 && (
        <Page size="A4" style={styles.page} orientation="landscape">
          <View style={styles.section}>
            <Text style={{ fontWeight: 'bold', fontSize: 12 }}>Spisak ucenika koji nisu primljeni u strucno zvanje {sanitizedCourseName}</Text>
            <Text style={{ fontWeight: 'bold', fontSize: 12, marginBottom: 10 }}>Ucenici koji imaju zvjezdicu, prema kriteriju Ministarstva za obrazovanje, nauku, kulturu i sport ZDK imaju pravo na BEZUVJETAN upis u prvi razred u školskoj 2024/2025. godini.</Text>
            <View style={styles.table}>
              <View style={styles.tableRow}>
                {/* 1st header cells */}
                <Text style={[styles.tableCell, styles.headerCell, { width: '30%' }]} colSpan={3}>Generalije</Text>
              ...
              {/* Rows for each student */}
              {secondPageStudents.map((student, studentIndex) => (
                <View key={studentIndex} style={styles.tableRow}>
                  {/* Cells for each student data */}
                  <Text style={[styles.tableCell, { width: '3%' }]}>{`${studentIndex + 1 + firstPageStudents.length}`}</Text>
                  <Text style={[styles.tableCell, { width: '16%', fontWeight: student.status === 'unconditional' ? 'bold' : 'normal' }]}>
                    {student.status === 'generation_best_student' && <Text style={styles.ugText}>(UG) </Text>}
                    {`${student.last_name || ''} ${student.name || ''}`}
                    {student.status === 'unconditional' && ' *'}
