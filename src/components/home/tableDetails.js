...
const TableDetails = ({ courseId }) => {
    ...
    const specialScoreNames = Object.keys(students[0]?.specialScores || {})
        .filter(key => {
    ...
    return (
        <div className="full-w overflow-x-auto">
            ...
            <PDFDownloadLink
                ...
                document={<PDFDocument students={students} specialScoreNames={specialScoreNames} courseId={courseId}/>}
                fileName="tabela.pdf"
            >
                {({ blob, url, loading, error }) => (loading ? 'Učitavanje...' : 'Preuzmi PDF')}
            </PDFDownloadLink>
            )}
            <table className="min-w-full border border-gray-200 mt-2">
                <thead className="bg-gray-50">
                    <tr className="border-b border-gray-200">
                        ...
                        {students[0]?.averageScores && Object.keys(students[0]?.averageScores).length > 0 && (
                            <>
                                <th colSpan={Object.keys(students[0]?.averageScores).length + 1} scope="colgroup" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">I-Opšti kriterij - Uzima se USPJEH od VI do IX razreda O.Š. zatim se sabere i pomnoži sa 3. (max: 60)</th>
                    ...
                    <tr>
                        {['RB', 'Ime i prezime', 'Osnovna škola', ...(students[0]?.averageScores ? Object.keys(students[0]?.averageScores) : []), 'SV (Opšti kriterij)', ...specialScoreNames, 'SV (posebni kriterij)', 'Federalno', 'Kantonalno', 'Općinsko', 'SV (specijalni kriterij)', 'Ukupno'].map(header => (
                        ...
                        <tr key={studentIndex}>
                            ...
                            {[`${student?.name} ${student?.last_name}`, student?.primary_school, ...(Object.values(student?.averageScores || {})), student?.sv, ...Object.values(student?.specialScores || {})
                                .filter((score, index) => {
                                    ...
                                    return classCode === "VIII" || classCode === "IX";
                                }), student?.sv2, ...(Object.values(student?.acknowledgmentPoints || {})), student?.sv3, student?.total].map((value, index) => (
